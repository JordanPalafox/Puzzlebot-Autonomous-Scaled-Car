...
class TurnRight(py_trees.behaviour.Behaviour):
    def __init__(self, name="TurnRight", node=None, angle=np.pi/2, angular_speed=0.3, linear_speed=0.1, use_odometry=True, duration_fallback=3.0):
        super().__init__(name)
        self.node = node
...
Behavior que gira 90° a la derecha mientras avanza a velocidad constante,
usando encoders en lazo cerrado para controlar el ángulo.

Ahora suscribe a /VelocityEncL y /VelocityEncR como Float32 en vez de Int32.
"""
...
class TurnRight(py_trees.behaviour.Behaviour):
    def __init__(self,
        ...
        # Variables de encoder (no resetear aquí; se inicializan en initialise)
        ...
        # Umbral de ángulo para terminar (~0.6°)
        self.angle_threshold = 0.01

    def setup(self, **kwargs):
        """
        Crear publisher / subscribers. Usamos Float32 para /VelocityEncL y /VelocityEncR.
        """
        if self.node:
            self.cmd_vel_pub = self.node.create_publisher(Twist, '/cmd_vel', 10)
            ...
            self.node.get_logger().info("[TurnRight] Setup completado (esperando encoders).")
        return True

    def left_encoder_cb(self, msg: Float32):
        """
        Callback para el encoder izquierdo (mensaje Float32).
        """
        self.left_count_raw = msg.data
        # Si ya teníamos un init para ambos, marcamos que hay datos
    ...
    def right_encoder_cb(self, msg: Float32):
        """
        Callback para el encoder derecho (mensaje Float32).
        """
    ...
    def ticks_to_radians(self, delta_ticks):
        """
        Convierte (float) delta de ticks de encoder a radianes de giro de rueda.
        """
        return (2.0 * math.pi * delta_ticks) / self.ticks_per_rev

    def compute_yaw_from_encoders(self):
        """
        Calcula el yaw actual del chasis en radianes:
          Δyaw = (R / L) * (θ_right - θ_left),
        donde θ_* = rotación de rueda en rad.
        """
    ...
    def wrap_to_pi(self, theta):
        """
        Normaliza un ángulo theta al rango [-π, π].
        """
        result = np.fmod((theta + np.pi), (2 * np.pi))
        if result < 0:
            result += 2 * np.pi
    ...
    def initialise(self):
        ...
        Se llama al empezar el giro:
         1) Si ya hay conteos “raw”, los tomo como init.
         2) Si no, espero hasta 1 s por ambos.
         3) Calculo yaw inicial y yaw objetivo (a la derecha, restamos target_angle).
         4) Reseteo PID.
        """
        # Resetear estado PID
        self.integral_angular   = 0.0
        ...
        # 1) Si ya vinieron mensajes, úsalo para init
        if self.left_count_raw is not None:
            self.left_count_init = self.left_count_raw
        if self.right_count_raw is not None:
            self.right_count_init = self.right_count_raw

        # 2) Si falta alguno, esperar hasta 1 segundo
        ...
        if self.left_count_init is None or self.right_count_init is None:
            if self.node:
                self.node.get_logger().error("[TurnRight] No se recibieron datos iniciales de encoders.")
            return

        # Ahora sí tenemos init para ambos
        self.has_encoders = True

        # 3) Calcular yaw inicial y yaw objetivo
        self.initial_yaw_enc = self.compute_yaw_from_encoders()
        # Al girar a la derecha, restamos target_angle
        self.target_yaw_enc = self.wrap_to_pi(self.initial_yaw_enc - self.target_angle)

        if self.node:
            self.node.get_logger().info(
                f"[TurnRight] initial_yaw={self.initial_yaw_enc:.3f} rad, "
                f"target_yaw={self.target_yaw_enc:.3f} rad"
            )
    ...
    def update(self):
        ...
        if self.has_encoders:
            ...
            # Log cada 0.5 s aprox.
            if self.node and (current_time - self.start_time) % 0.5 < 0.02:
                self.node.get_logger().info(
                    f"[TurnRight] yaw_act={current_yaw_enc:.3f} rad, error={angle_error:.3f} rad"
                )
            ...
            # Si el error es significativo, forzamos un mínimo en la dirección correcta.
            if abs(angle_error) > self.angle_threshold:
                min_speed = 0.05
                if angle_error < 0:
            ...
            if abs(angle_error) < self.angle_threshold:
                ...
                if self.node:
                    self.node.get_logger().info(
                        f"[TurnRight] Giro completado: yaw_final={current_yaw_enc:.3f} rad"
                    )
                return py_trees.common.Status.SUCCESS

        else:
            # Fallback: girar por tiempo fijo si no hay encoders
            ...
            if elapsed < self.duration_fallback:
                ...
                self.stop_robot()
                if self.node:
                    self.node.get_logger().info("[TurnRight] Fallback completado.")
                return py_trees.common.Status.SUCCESS
    ...
    def terminate(self, new_status):
        self.stop_robot()
        if self.node:
            self.node.get_logger().info("[TurnRight] Terminando comportamiento.")
