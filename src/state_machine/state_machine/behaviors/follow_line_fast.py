...
class FollowLineFast(py_trees.behaviour.Behaviour):
    def __init__(self, name="FollowLineFast", node=None):
        ...
        self.linear_speed = 0.06      # Velocidad lineal base
        self.max_angular_speed = 0.18 # Velocidad angular máxima
    ...
    def initialise(self):
        ...
        """
        if self.node:
            self.node.get_logger().info("🚀 FollowLineFast: iniciando seguimiento de línea lento avanzado")
        ...
        try:
            ...
            if self.frame_count % 3 == 0:
                ...
                if final_centroid is not None:
                    ...
                    cv2.putText(control_view, 'Fast MODE', (10, 25),
            ...
            if self.node:
                self.node.get_logger().error(f"FollowLineFast: error en procesamiento: {e}")
            self.stop_robot()
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            if self.node:
                self.node.get_logger().error(f"FollowLineFast: error al convertir imagen: {e}")
