...
class FollowLineFast(py_trees.behaviour.Behaviour):
    def __init__(self, name="FollowLineFast", node=None):
        ...
        # Parámetros PID, velocidades, ROI, etc.
        self.roi_height = 20
        self.roi_width = 200
        self.threshold = 75
        self.kp = 0.2
        ...
        # Flag para saber si este comportamiento está activo
        self.is_active = False

        # Aquí guardaremos la última imagen recibida
        self.latest_frame = None

    def setup(self, **kwargs):
        # 1. Crear publicador cmd_vel
        if self.node:
            self.cmd_vel_pub = self.node.create_publisher(Twist, '/cmd_vel', 10)

            # 2. Crear suscripción a la cámara (CompressedImage)
            ...
            self.node.get_logger().info("🚀 FollowLineFast: suscripción a /video_source/compressed creada.")
        return True

    def initialise(self):
        if self.node:
            self.node.get_logger().info("🚀 FollowLineFast: iniciando seguimiento rápido")
        # Activar el procesamiento
        self.is_active = True

        # Reset de variables al entrar en el estado
        self.last_error = 0.0
        self.smoothed_error = 0.0
        self.integral_error = 0.0
        ...
        # Inicializar la ventana de debug (solo una vez)
    ...
    def update(self):
        # Si no hemos recibido aún ningún frame, no hacemos nada
        if self.latest_frame is None:
            return py_trees.common.Status.RUNNING

        # Convertir el último frame a BGR para procesarlo
        try:
            # CompressedImage -> cv2 image en BGR
            image = self.latest_frame.copy()
        except Exception as e:
            if self.node:
                self.node.get_logger().error(f"Error copiando frame: {e}")
            return py_trees.common.Status.RUNNING

        # Procesamiento de línea: recortamos ROI, umbral, contornos, PID, etc.
        ...
        # Encontrar centroid de la línea
        ...
        if centroid is not None:
            ...
            if self.frame_count % 3 == 0:
                ...
                cv2.line(debug_roi, (binary.shape[1] // 2, 0), (binary.shape[1] // 2, binary.shape[0]), (255, 0, 0), 2)
                cv2.putText(debug_roi, 'FAST MODE', (10, 25),
    ...
    def terminate(self, new_status):
        # Al salir de este comportamiento
        self.is_active = False
        self._stop_robot()
        # No destruimos la suscripción: la dejamos para poder reutilizarla más adelante
        if self.node:
            self.node.get_logger().info("🛑 FollowLineFast: terminado, robot detenido")

    def image_callback(self, msg):
        """Callback de ROS 2: solo guarda el último frame en self.latest_frame"""
        if not self.is_active:
            return

        try:
            cv_image = self.bridge.compressed_imgmsg_to_cv2(msg, 'bgr8')
            # Guardamos el frame, pero no lo mostramos aquí
            self.latest_frame = cv_image
        except Exception as e:
            if self.node:
                self.node.get_logger().error(f"Error al convertir imagen: {e}")

    def _stop_robot(self):
        """Helper para detener robot publicando Twist(0,0)"""
        if self.cmd_vel_pub:
            self.cmd_vel_pub.publish(Twist())
